...
async def create_rag_db(payload: RagPayload, customer_id=Depends(CustomerId())) -> RagConfig:
    ...
    if config:
        raise HTTPException(status_code=409, detail='RAG database already exists')

    config = await store.create_from_urls(payload, customer_id)
...
@api_version(1)
@router.put('/rag')
async def update_rag_db(payload: RagPayload, customer_id=Depends(CustomerId())) -> RagConfig:
    """
    Create / update a RAG database.
    """
