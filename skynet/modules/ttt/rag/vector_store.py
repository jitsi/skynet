...
from skynet.env import embeddings_model_path, openai_embeddings_api_base_url, use_vllm
from skynet.logs import get_logger
from skynet.modules.ttt.assistant.v1.models import RagConfig, RagPayload, RagStatus
from skynet.modules.ttt.rag.web_crawler import crawl
...
class SkynetVectorStore(ABC):
    embedding = (
        OpenAIEmbeddings(
            api_key='placeholder', base_url=f'{openai_embeddings_api_base_url}/v1', model=embeddings_model_path
        )
        if use_vllm
        else OllamaEmbeddings(base_url=openai_embeddings_api_base_url, model=embeddings_model_path)
    )
    tasks = set()
